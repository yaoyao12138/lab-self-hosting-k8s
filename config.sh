# The version of kind
KIND_VERSION=${KIND_VERSION:-v0.11.1}
# The version of kubectl
KUBECTL_VERSION=${KUBECTL_VERSION:-v1.17.11}
# The version of helm
HELM3_VERSION=${HELM3_VERSION:-v3.5.3}
# The version of nfs-subdir-external-provisioner 
NFS_PROVISIONER_VERSION=${NFS_PROVISIONER_VERSION:-4.0.13}
# The version of Instana
INSTANA_VERSION=${INSTANA_VERSION:-205-2}
# Instana settings
INSTANA_HOST=         # Your Instana hostname
INSTANA_DB_HOST=      # Your Instana db hostname
INSTANA_LICENSE=      # Your Instana license file
INSTANA_DOWNLOAD_KEY= # Your Instana download key
INSTANA_SALES_KEY=    # Your Instana sales key
NFS_HOST=         # Your NFS hostname
# The required images to be pulled and loaded
# REQUIRED_IMAGES=()
